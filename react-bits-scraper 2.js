...
class ReactBitsScraper {
  ...
  async scrapeRepositoryOverview() {
    ...
    const overview = await this.page.evaluate(() => {
      ...
      const statsElement = document.querySelector('[data-view-component="true"] .BorderGrid-row');
      if (statsElement) {
        const watchersLink = document.querySelector('a[href*="/watchers"]');
        const starsLink = document.querySelector('a[href*="/stargazers"]');
        const forksLink = document.querySelector('a[href*="/forks"]');
        
        data.watchers = watchersLink?.textContent?.trim() || '0';
        data.stars = starsLink?.textContent?.trim() || '0';
        data.forks = forksLink?.textContent?.trim() || '0';
      }

      // Repository description and topics
      const descriptionEl = document.querySelector('[data-repository-hovercards-enabled] p');
      ...
      // License information
      const licenseLink = document.querySelector('a[data-analytics-event*="license"]');
      data.license = licenseLink?.textContent?.trim() || 'Not specified';

      // Commit count and recent activity
      const commitsLink = document.querySelector('a[href*="/commits/"]');
      data.commits = commitsLink?.textContent?.trim() || '0';

      // Issues and PRs
      const issuesLink = document.querySelector('a[href*="/issues"]');
      const prsLink = document.querySelector('a[href*="/pulls"]');
      data.issues = issuesLink?.textContent?.match(/\d+/)?.[0] || '0';
      data.pullRequests = prsLink?.textContent?.match(/\d+/)?.[0] || '0';
  ...
  async exploreDirectory(path = '') {
    ...
    const directoryData = await this.page.evaluate((currentPath) => {
      ...
      fileRows.forEach(row => {
        ...
        const sizeCell = row.querySelector('[role="gridcell"]:last-of-type');
